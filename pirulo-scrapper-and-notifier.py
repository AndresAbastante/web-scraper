...
for j in tqdm (range (4, maxpages), desc='Scrapping...'):
...
olddf = pd.read_csv('mtgpirulo_japanese_stock.csv')
#newdf = pd.read_csv(csvfilename)

mergeddf = pd.merge(olddf,newdf, on='Product', how='outer')
mergeddf.to_csv('merged.csv')
#dfcomparer = newdf[-olddf.apply(tuple,1).isin(newdf.apply(tuple,1))]
